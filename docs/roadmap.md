...
- [x] **Look back at previous days** — the day strip was previously display-only; tapping another day did nothing, so there was no way to review or fix a past entry. Tapping a day chip in this week's strip now switches the whole Today tab (logging, readiness, cardio, swap, custom exercises) to that date, with a "Today" button to jump back and a visual distinction between the day you're *viewing* (bordered) and the *real* today (underlined label), since they can differ. Scoped to the current week's strip only, no prev/next-week navigation added. Live streak/XP calculations are deliberately unaffected by which day you're viewing — only the actual current date can complete "today."
